...
class AudioDataset(Dataset):
    ...
    def __init__(self, 
        ...
        # C-2.3: Single source of truth — _build_file_list handles synthetic flag
        ...
        # Build file list (handles synthetic internally)
...
class Slakh2100YourMT3Dataset(AudioDataset):
    ...
    def _build_file_list(self) -> List[str]:
        """Override: short-circuit on synthetic to avoid FileNotFoundError (C-2.4)."""
        if self.synthetic:
            return [f"synthetic_{i}" for i in range(100)]
...
class MUSDB18HQDataset(AudioDataset):
    ...
    def _build_file_list(self) -> List[str]:
        ...
        if self.synthetic:
            # Must not reference self.file_list — __init__ assigns it from this return value.
            return [f"synthetic_{i}" for i in range(100)]
        file_list = []
        for entry in os.listdir(self.root_dir):
            track_dir = os.path.join(self.root_dir, entry)
    ...
    def _get_real_item(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # C-2.7: sum() on dict values returns 0-d array; use explicit unpack
        total = sum(stem_wavs[k] for k in stem_wavs)
        mixture = total / len(stem_wavs)
