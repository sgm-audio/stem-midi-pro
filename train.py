#!/usr/bin/env python3
"""
Training script for Stem+MIDI Pro with Canadian artist dataset support
"""
...
from data.canadian_datasets import get_canadian_data_loaders
...
def main():
    ...
    parser.add_argument(
        "--data-config", type=str, required=True, help="Path to data configuration file"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    # Initialize model
    print("Initializing Stem+MIDI Pro model...")
    model = StemMidiModel(model_config)

    # Setup data loaders
    print("Loading Canadian artist datasets...")
    train_loader, val_loader, test_loader = get_canadian_data_loaders(data_config)
    ...
    logger = TensorBoardLogger(
        save_dir=args.output_dir, name="stem-midi-pro", version=None
    )
    ...
    trainer = pl.Trainer(
        ...
        precision=16
        if model_config.get("training", {}).get("precision") == "fp8"
        else 32,
    ...
    print(f"Logs saved to {args.output_dir}/stem-midi-pro/*")
