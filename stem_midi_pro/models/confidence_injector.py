class ConfidenceInjector:
    ...
    def inject_midi_metadata(self, midi_events: list, confidence: torch.Tensor, 
                          stem_energy: torch.Tensor) -> dict:
        """
        Args:
        ...
        for event in midi_events:
            ...
            conf_score = confidence[onset_idx].item() if onset_idx < len(confidence) else 0.0
            energy = stem_energy[onset_idx].item() if onset_idx < len(stem_energy) else 0.0
            
            # Cross-modal alignment penalty: low confidence + low energy = likely artifact
            alignment_score = conf_score * (1.0 / (1.0 + torch.exp(-torch.tensor(energy * 10)))).item()
        ...
        return {
            ...
            'summary': {
                ...
                'low_confidence_count': (confidence < self.min_confidence).sum().item() if len(confidence) > 0 else 0,
