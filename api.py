...
SUPPORTED_BIT_DEPTHS = {
    16,
    24,
}  # Note: soundfile doesn't directly give bit depth, we'll infer
...
def validate_audio_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if bit_depth and bit_depth not in SUPPORTED_BIT_DEPTHS:
            ...
            print(
                f"Warning: Bit depth {bit_depth} not in preferred list {SUPPORTED_BIT_DEPTHS}"
            )
...
def create_response_zip(outputs: Dict) -> io.BytesIO:
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        sf.write(
            bass_buffer, bass_stem, model_config["audio"]["sample_rate"], format="WAV"
        )
